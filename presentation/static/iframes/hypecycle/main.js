...
const CONST = { canvas : { width : 2500 , height : 1000,
                            margin : { top   : 0, right: 100, bottom:80, left:95}, //margin settings for Axes
                            graph  : { shift : {left: 0, top: -700},
                                        labels: {title: "",
                                            xAxisLabel: "Time",
...
const svgHC = svg.append('g').attr('id', 'HypeCycleCurveGroup')
    .attr('transform', `translate(${CONST.canvas.graph.shift.left},${CONST.canvas.graph.shift.top})`)
...
function drawHypeCyclePhases(data) {
    svgPhases = svgHC.append('g').attr('transform', `translate(${150},${30})`)
    data.map((p,i) =>{
...
function drawHypeCyclePoints(data) {
    ...
    data.points.forEach(o => { //o = observation
        ...
        svgHC.append('text').text(o.label).attr('x', p.x + o.dx).attr('y', p.y + o.dy)
            .attr('text-anchor', () =>  (o.position == 'left') ? 'end': 'start')
            .attr('class','callout')

        svgHC.append('circle').attr('r', 15)
